...
if(runOnce){
  ...
  data.txaDESCRICAO_CONFIRMAR_SOLICITACAO_GERAL = data.txaDESCRICAO_ATENDER_SOLICITACAO_GERAL;
   data.txaDESCRICAO_SOLICITACAO_NAO_ATENDIDA = data.txaDESCRICAO_SOLICITACAO_NAO_ATENDIDA2;
...
function configurarConfirmarSolicitacao() {
  ...
  const camposNaoAtendida = [
    "txaDESCRICAO_SOLICITACAO_NAO_ATENDIDA",
    "uplCONFIRMAR_ANEXO_NAO_ATENDIDO"
  ];

  if (data.sltSELECT_CONFIRMAR_SOLICITACAO_SELECT === "Não") {
    exibirCampos(camposNaoAtendida);
  } else if (data.sltSELECT_CONFIRMAR_SOLICITACAO_SELECT === "Sim") {
    ocultarCampos(camposNaoAtendida);
  }
}
...
if(submitting){
  if(data.sltSELECT_CONFIRMAR_SOLICITACAO_SELECT === "Sim"){
     data.Atendido = true     
     }
    else if(data.sltSELECT_CONFIRMAR_SOLICITACAO_SELECT === "Não"){
     data.Atendido = false 
